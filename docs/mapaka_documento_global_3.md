...
- **Temps de pantalla — cambio de modelo.** Se decidió que no funcione con generación diaria (el planteamiento original), sino exactamente igual que el dinero: cada hijo tiene un número de minutos que se le asignan una vez al mes (campo "Minuts de pantalla per mes" en Fills), se van gastando con las sesiones NFC, y lo que no se gasta se queda acumulado para el mes siguiente — como el dato móvil de una tarifa de telefonía, no como un cupo que se vacía cada día. Es coherente con la regla de oro del proyecto (nunca un saldo fijo, siempre suma de movimientos) y evita que haya que tocar la app a diario cuando la familia no usa la tableta con frecuencia. **Nota (verificación Prompt 16, 2026-08-29):** esta decisión nunca se terminó de implementar en el backend — el motor real seguía acreditando `ScreenTimeRule.baseMinutes` cada día (`DailyBaseCreditor`), ~28 veces más de lo que el padre configuraba. Corregido moviendo la acreditación al mismo acto que "Generar la paga del mes" (`AllowanceGenerationService.confirm()`); detalle técnico en `mapaka_prompts_code_6.md`, ajust posterior 5.
...
Todo lo anterior está decidido y documentado. El siguiente paso es la implementación: abrir `mapaka_prompts_code.md` y ejecutar los 16 prompts en orden con Code, revisando el resultado de cada uno antes de pasar al siguiente. Ese documento contiene las instrucciones técnicas exactas; este documento es el que explica el porqué de cada una, para volver a él cuando haga falta recordar una decisión o poner a alguien nuevo en contexto.
...
## 12. Ronda de ajustes sobre la app real (agosto 2026)

Con los Prompts 1-10 ya implementados y en revisión (Code trabajando ya hacia el Prompt 12), la primera vez que se vio la app funcionando de verdad surgieron varios ajustes que no se habían previsto en el diseño original. Se recogieron todos, se propuso una solución para cada uno (con mockups comparativos donde hacía falta decidir entre varias opciones visuales — `mapaka_mockup_v2.html`) y se cerraron en el **Prompt 15** de `mapaka_prompts_code.md`. Resumen de lo más relevante:

- **Alertas en el panel de adultos**: una campanita con contador en Resum familiar avisa, a partir del día 1 de cada mes, si todavía no se ha generado la paga de ese mes — calculado siempre al vuelo a partir del último movimiento, nunca guardado como estado nuevo.
- **Recompensa de tareas más clara**: cada tipo de recompensa (dinero, tiempo de pantalla) tiene su propio interruptor, en vez de confiar en que un campo se quede a 0 sin más explicación.
- **Tareas Extra sin asignación fija, con colaboración entre hermanos**: la tarea aparece disponible para todos; el primero que la marca como hecha indica si ha recibido ayuda y, si es así, elige qué hermanos han colaborado — la recompensa se reparte a partes iguales entre todos los participantes, y la tarea queda bloqueada para el resto en el momento en que se envía, no cuando el padre la aprueba. Es el cambio de modelo más grande de esta ronda: una finalización de tarea pasa de ser siempre "1 tarea = 1 hijo" a admitir un grupo de varios hijos aprobado de una sola vez.
- **Penalización en tareas de Responsabilidad**: al ser obligatorias, admiten recompensa en 0, y además permiten configurar una penalización (dinero y/o tiempo) que el padre aplica manualmente desde un listado de "tareas incompletas" — nunca de forma automática, para no penalizar un día de enfermedad o vacaciones.
- **Minutos de pantalla sin restricción de múltiplos de 5**: cualquier número entero, en cualquier campo de minutos de la app.
- **Sesión del hijo**: se corrigió el solapamiento de texto en la pantalla de tiempo de pantalla; cada hijo tiene ahora un avatar (círculo de su color, con su inicial o un icono de líneas blancas si lo personaliza) visible junto al saludo en todas sus pantallas, y editable desde la barra de navegación (eligiendo color entre una paleta cerrada que excluye blancos, para que el icono siempre tenga contraste). Los minutos de pantalla se definen ahora "por semana" en el formulario de Fills, aunque por debajo se sigue generando una vez al mes (el sistema convierte automáticamente semana × 4) — sin añadir ninguna tarea programada nueva.
- **Resum familiar**: las tarjetas de cada hijo (y de los adultos) muestran ahora a la vez el total, lo disponible para gastar y el ahorro — antes solo se veía el importe para gastar. La lista de movimientos deja de crecer sin límite: se filtra por semana, mes o hijo, con acceso a un listado completo aparte. El color elegido para cada hijo se aplica de forma consistente en su avatar, su tarjeta de resumen y su ficha de Fills. El icono de Tasques en la barra de navegación del hijo muestra un aviso cuando tiene tareas nuevas asignadas, con el mismo tratamiento que el aviso de Aprobaciones pendientes de los padres.
- **Donaciones y objetivos con porcentaje propio** (el cambio de arquitectura más importante de esta ronda): los padres pueden registrar manualmente una donación de un familiar hacia el objetivo de ahorro de un hijo, sin pasar por el reparto gastar/ahorro. Y al crear un objetivo, además del importe a alcanzar, se define qué porcentaje de puntos se resta directamente del "para gastar" del hijo mientras el objetivo esté activo (ejemplo: 70 % gastar / 30 % ahorro, un objetivo al 10 % deja 60 % gastar / 30 % ahorro / 10 % objetivo). Esto obliga a que el cálculo de reparto de dinero, que hasta ahora era siempre binario (gastar/ahorro), pase a repartirse en tantas partes como objetivos activos tenga el hijo, en los tres sitios donde se genera dinero (paga mensual, bonificaciones, aprobación de recompensa de tareas) — se centraliza en un único cálculo compartido para no duplicar la lógica. Al completarse un objetivo, su porcentaje se libera solo, sin que el padre tenga que hacer nada.

Detalle técnico completo en el Prompt 15 de `mapaka_prompts_code.md`, y en los puntos 18-30 añadidos a la lista de verificación del Prompt 16.
