...
- **Decisiones tomadas durante la implementación, no explícitas en el diseño original:**
  - El interruptor "Paga personalitzada" necesitaba una forma de *desactivarse* sin perder el histórico — se añadió `DELETE /api/children/{childId}/allowance-rule` (desactiva la regla propia del hijo, no la borra físicamente, y el hijo vuelve a depender de la regla general) y un campo `hasCustomAllowance` en el detalle del hijo para que el frontend sepa el estado inicial del interruptor.
  - El componente `switch` que los documentos daban por "ya existente y reutilizable" en realidad estaba duplicado como CSS local en cada pantalla (Configuració). Se extrajo como componente compartido `BaseSwitch.vue` (junto con `FormRow.vue`) y ambos ya viven en el sistema de diseño del Prompt 2.
  - La corrección "Cancel·la nunca en rojo" se aplicó no solo al formulario de Tasques (su alcance original) sino a **todos** los formularios de la app con un botón de solo cancelar — Configuració (reglas de paga, reset de PIN, alta de adulto), Resum (cancelar paga generada) y Objectius del hijo — mediante una nueva variante `ghost` en `BaseButton`, para no dejar la app con dos criterios de color distintos.
  - El reparto gastar/estalviar (tareas y Bonificacions) comparte una única utilidad `MoneySplit` + `AllowanceRuleService.resolveSpendingPercentage`, en vez de reimplementar el cálculo del % vigente en cada sitio.
