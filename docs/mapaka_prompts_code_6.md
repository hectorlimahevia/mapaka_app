...
- **Els objectius actius queden plegats darrere un `BaseSwitch`** (reaprofitant el component ja existent, no un xip ni un acordió de fletxa) — la fila "Objectius (N)" només es renderitza si el fill té algun objectiu `ACTIVE`; amb el switch obert, cada objectiu mostra nom, barra de progrés (color del fill), imports i percentatge, i un botó "Donar" propi (abans compartien fila amb el nom). **El switch sempre comença tancat** (`ref<Record<string,boolean>>` per childId, sense persistir enlloc) tal com demanava l'especificació.
- **La secció d'objectius `COMPLETED` (trofeu daurat) de l'ajust posterior 3 s'ha retirat d'aquesta card** — l'especificació d'aquest redisseny només cobria els objectius `ACTIVE` dins del desplegable; els objectius assolits d'un fill ja es poden consultar des de la seva pròpia pantalla d'Objectius. Se signala aquí perquè no quedi silenciat: si es vol tornar a mostrar-los al Resum familiar caldrà decidir on encaixen dins d'aquesta nova estructura.
