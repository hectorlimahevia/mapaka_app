...
3. Crea components base reutilitzables amb aquests tokens: BaseButton (variant primary/accent/danger/**ghost** — la variant `ghost` es va afegir en implementar el Prompt 10: cap acció de "Cancel·la" que només descarta un formulari ha de ser vermella, el vermell queda reservat a accions destructives reals), BaseCard, AmountDisplay (força font-variant-numeric: tabular-nums i pes 800+), BadgeCounter, **BaseSwitch** (interruptor arrodonit — es va detectar en implementar el Prompt 9 que el "switch de marca" que els prompts donaven per existent i reutilitzable en realitat estava duplicat com a CSS local a cada pantalla; es va extreure com a component perquè Fills i Tasques el poguessin reutilitzar de veritat).
...
- **Fills:** gestió de perfil i edat per fill, més:
  - **Paga — interruptor "Paga personalitzada per a aquest fill"** (component compartit `BaseSwitch.vue` — es va extreure del CSS local de Configuració perquè realment fos reutilitzable, no només "el mateix aspecte" copiat a mà). Per defecte, **desactivat**: es mostra un bloc de només lectura "Aplica la regla general: X € al mes (segons edat), Y% per gastar / Z% per estalviar" — el número surt de la regla general per franja d'edat que correspongui, mai un camp editable, perquè no hi hagi dos llocs on definir el mateix import (aquest era exactament el problema de la v1). Si s'activa l'interruptor, apareixen dos camps editables — **Paga mensual (€)** i **% per gastar** (la resta va a estalvi, mateix patró que ja hi havia) — que creen o actualitzen una fila a `allowance_rules` amb `child_id` establert, prevalent sobre la regla general (secció 8.2 de Família+.pdf). **Desactivar-lo** (decisió presa en implementar, no explícita al disseny original) crida `DELETE /api/children/{childId}/allowance-rule`, que desactiva la regla pròpia sense esborrar-la (mateix criteri d'històric que la resta de l'app) — el fill torna a dependre de la regla general. `GET /api/families/{id}/children/detail` inclou un nou camp `hasCustomAllowance` perquè el frontend sàpiga l'estat inicial de l'interruptor.
...
- **Decisió presa en implementar, no explícita aquí:** aquest mateix càlcul de repartiment el necessiten també les Bonificacions del Prompt 9 — en comptes de duplicar-lo, viu en una única utilitat `MoneySplit` + `AllowanceRuleService.resolveSpendingPercentage(child)` (paquet `allowance`), que `ApprovalService` i `AdjustmentService` criden per igual.
...
- **Formulari "Nova tasca" / "Edita tasca" — maquetació en 3 passos dins la mateixa targeta** (es van comparar 3 opcions — una columna amb blocs, dues columnes a escriptori, i passos — es va triar aquesta): pestanyes "1. Bàsic", "2. Recompensa", "3. Assignació" amb un indicador animat que es desplaça entre elles, reproduint el mateix patró (transform + transition) que l'indicador de la navegació inferior.
  ...
  - **Pas 2, Recompensa:** només dos camps — **Diners (€)** i **Minuts de pantalla** (com a mínim un dels dos > 0). Sota els camps, mostra una previsualització de només lectura del repartiment ("Es repartirà en aprovar-la: X € gastar / Y € estalvi") calculada amb el percentatge del fill assignat (si n'hi ha més d'un amb percentatges diferents, mostra un rang o el del primer fill seleccionat amb un avís que pot variar per fill). El camp "Requereix aprovació" és el component compartit `BaseSwitch.vue`, no un checkbox del navegador ni un tercer desplegable.
  - **Pas 3, Assignació:** píndoles de selecció múltiple dels fills.
  - Botons "Cancel·la" (variant `ghost` de `BaseButton`, discret — mai vermell sòlid: el vermell queda reservat a accions destructives reals) i "Desar" (primari) fixos sota les pestanyes, visibles independentment del pas actiu.
...
- **Abast ampliat en implementar:** la variant `ghost` es va aplicar a tots els botons "Cancel·la" de l'app (Configuració, Resum, Objectius del fill), no només al d'aquest formulari — l'abast original del disseny era només Tasques, però deixar vermells la resta hauria creat dos criteris de color diferents dins la mateixa app.
